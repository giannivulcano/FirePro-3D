...
- [ ] Bug: PDF interactive placement (via import dialog) loses vector geometry on refresh — `_commit_place_import` bakes vectors into QGraphicsItems but `import_pdf` re-renders as raster. PDFs placed via import dialog show as raster after refresh/reload instead of vectors. Separate from DXF fix. `model_space.py:_commit_place_import`, `model_space.py:import_pdf` [type:Bug] [P2] [subject:CAD]
