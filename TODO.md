...
- [ ] Spec session: grid system architecture — two grid classes (`GridLine` 302 LOC vs `GridlineItem` 372 LOC), auto-numbering (global counters, multi-letter scheme AA/AB), bubble visibility per-end, pull-tab grips, lock/unlock, `ItemIgnoresTransformations` on bubbles, serialization format. Also covers elevation view creation lifecycle (`ElevationScene` 1233 LOC projection algorithm, cardinal-axis coordinate mapping, Z-range entity filtering, gridline caching) [type:Backlog] [P1] [subject:Architecture]
- [ ] Spec session: scale calibration & underlay workflow — two-point calibration UI, unit parsing edge cases (ft-in, mm, m formats), pixels_per_mm computation, linked-file underlay persistence (re-read on load, file-not-found fallback, path resolution), multi-page PDF page selection, rotation/scale origin semantics. Modules: `calibrate_dialog.py` (263 LOC), `underlay.py` (67 LOC) [type:Backlog] [P1] [subject:Architecture]
