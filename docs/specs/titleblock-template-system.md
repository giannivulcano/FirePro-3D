...
4. **One editor window does it all:** ribbon Draft tab → "Title Block". Library rail ("Name (SIZE)") with a `+/−/⧉` header, component tabs **Overview / Fields / Arrangements** (the Drawing Area border lives in the Overview info rail — 2026-09-15), and footer **Apply Template / Save Template / Close**.
...
25. **Revision table true columns.** The revision table renders as three real columns computed at paint time (`TitleBlockTemplateItem.paint`; the solver still only reserves cell height + supplies `cell_revision_rows`). **Rev.** (centre) and **Date** (right) auto-size to their widest content *including the header* (via `_rev_text_width_mm`, measured at `TB_REV_CAP_MM`); **Description** (left) fills the remainder (Rev+Date capped at ¾ of the cell width so Description never collapses). Header and data share each column's justification (via `_draw_text_mm(..., align=…)`), so headers align to their data. No vertical dividers — whitespace between columns, horizontal row rules only. The header renames **"No" → "Rev."** in both the render and `RevisionsDialog._HEADERS`. Replaces the prior single faked-space string.
26. **Revision dates: ISO storage + project display format.** Revision `date` values are **stored ISO `YYYY-MM-DD`** in `Sheet.revisions` and rendered in a **project-scoped display format** — `project_info["date_format"]` (default `MM/DD/YYYY`; one of the `DATE_FORMATS` labels) — applied at value-build time (`build_field_values` formats a copy via `format_date_display`; storage untouched). The **RevisionsDialog** edits dates with a `QDateEdit` picker (empty "—" allowed; an unparseable legacy value is preserved verbatim until the user actively picks a new date — `parse_date_to_iso` is lenient and returns `None` = keep raw) and carries the project **Date format** dropdown (writes `project_info["date_format"]`; a format-only change re-renders + dirties, non-undoable like Display-Manager settings). New projects inherit the format from `default.fpdt`'s `project_info` (captured by "Save as default").
