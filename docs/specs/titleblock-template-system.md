...
9. **Value scoping** (grill): project-scoped = Company, Project, Address, Drawn By, Checked By; sheet-scoped = Title, Drawing No, Rev, Date. Auto = Scale, `Sheet No` (reserved for multi-sheet; **not** in the known-key set, so a hand-typed `@[Sheet No]` renders literally + warns — consistent between previews and sheets), auto-Date ("Date (auto)").
