...
verified-commit: d313a10   # + Sprinkler/DA/hydraulic domain slice landed (slice 6); prior: Pipe/Node (slice 5), slices 1 (A+C), 2 (B), 4a+4b (codec), Underlay slice; census @ 3a99b63
applies-to:
...
The end state is a thin `Model_Space` core plus scene-referencing collaborators. Landed collaborators: `UnderlayController` (underlay slice), `PipeNetworkController` (slice 5), `SprinklerWorkflowController` (slice 6 — supersedes the provisional `SprinklerWorkflow`/`DesignAreaController` name), and the `NetworkCodec` helper unifying the dual serializers. Remaining candidate collaborators (names provisional, to be settled in grill): `GeometryDrawingController`, per-element `*PlacementController`s (wall/floor/roof/opening/room/gridline), `PlacementInputCoordinator` (ALIGN+HUD+variants+templates).
