...
| **Block system** (definition/instance flyweight, library, Manager, browser, placement) | `specs/block-system.md` | `block_definition.py`, `block_instance.py`, `block_library.py`, `block_manager.py`, `blocks_browser.py`, `app_data.py`, `model_space.py` (registry + placement + make-from-selection + Manager API), `scene_io.py` (embed), `main.py` (ribbon/dock) | **partial** (S1–S4 built — flyweight def/instance + 2-tier `.fpdb` library + Manager; retired `BlockItem`; S5 icons pending; thumbnails + Editor/import/attributes/paper-space = v2) |
