...
`DesignAreaBadge` — a child `QGraphicsItem` of its DesignArea rendering the AHJ "DESIGN CRITERIA" table on the drawing. Sized in model-space mm via the `DA_BADGE_*` constants (`constants.py`); its unit spellings (`usgpm`, `ft²`) are the deliberate AHJ exception to project units (`units-and-formatting.md §6`). Locked at the 2026-07-14 mockup gate.
