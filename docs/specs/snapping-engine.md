...
> **Source files:** `firepro3d/snap_engine.py`, `firepro3d/model_view.py`, `firepro3d/model_space.py`, `firepro3d/underlay_import_dialog.py`, `firepro3d/constants.py` (`SNAP_MAX_*` per-item caps). `firepro3d/handle_snap.py` consumes this engine's collectors but is governed by [`selection-manipulator.md`](selection-manipulator.md).
> **Date:** 2026-04-07
...
> **Impl note (2026-09-24 — snap polish, `feat/snap-polish`, last-verified 2026-09-24, verified-commit `892cf76`):** **perpendicular-from** (§4: PER is the foot from the placement start point when one exists), **curve accuracy** (§5: circles judged on `rect()`, arcs exact, ellipses/splines/generic paths projected onto the flattened curve with a spatial cull — §6.1), **text and block snap points** (§5 TextItem / BlockInstance rows; TextItem no longer skipped in phase 1), and the **one-picker ALIGN seam + weak-foot rule** (§14.5). The retired `HatchItem` row is gone. *(Seam round 2026-09-25, last-verified 2026-09-25, verified-commit `d36af0a`: the §14.5 handle-snap `held` note is now a pointer to its owner, align-placement §3.)* *(Smoke round A 2026-09-25, last-verified 2026-09-25, verified-commit `e044d4d`: §4 — no start point → `nearest` only, nearest marker white, perpendicular-from on the 2D-primitive 1st→2nd-click step; §14.5 weak types always `{nearest}`.)*
...
|---|---|---|---|---|---|---|
| **endpoint** | square | `#ffff00` yellow | `END` | 1 | Line endpoints, polyline vertices, rectangle corners, arc start/end, wall centerline ends, wall face corners, generic path vertices, text frame-box corners, block stroked on-curve vertices (per-item detail: §5) | works |
| **midpoint** | triangle | `#00ff88` green | `MID` | 2 | Line midpoints, polyline segment midpoints, rectangle edge centers, wall centerline midpoint, wall face-edge midpoints, arc angular midpoint, text frame-box edge midpoints | works |
...
| **center** | circle | `#00eeee` cyan | `CEN` | 3 | Circle/ellipse centers, rectangle centers, arc centers, text frame-box centres, block insertion origin | works |
| **quadrant** | diamond | `#ff8800` orange | `QUA` | 5 | Circle 0°/90°/180°/270° points, arc quadrant points within angular range | works |
| **perpendicular** | right-angle | `#ff00ff` magenta | `PER` | 4 | Foot of a perpendicular onto any line/segment/wall edge/rectangle edge/polyline segment/arc/circle/ellipse/spline/generic path, and onto underlay geometry. **With a placement start point** (`find(from_point=…)`) it is the foot **from that start point** (AutoCAD PER): unclamped onto a segment and omitted when it falls off the segment; radial onto a circle/arc (both points where the line through the centre and the start point meets the circle; an arc keeps only those within its range). The seam passes a start point, once the placement's first point exists, in modes `draw_line`, `polyline`, `wall` (line primitive), `pipe`, `floor` (polygon primitive) and `roof`, and — only on the step between the 1st and 2nd click, while the reference line runs from the first click — `draw_rectangle` (side step), `draw_circle` (radius), `draw_ellipse` (major axis), `draw_arc` (step 1) and `draw_spline` (from the last control point) (`Model_Space._snap_from_point`). **Otherwise** (no start point, or any other mode/step) there is **no** `perpendicular` candidate at all — the cursor foot is `nearest` (2026-09-25 smoke decision). | works |
| **tangent** | tangent-circle | `#88ff00` lime | `TAN` | 6 | Tangent point from the cursor to a full circle or an `ArcItem` (arc: only tangent points on the visible arc) | works |
| **nearest** | cross | `#ffffff` white | `NEA` | 7 | Closest point to the cursor on a segment / curve (always the cursor foot, clamped onto the segment) — emitted independently of `perpendicular` (§5 note 1); with no placement start point it is the only foot snap | works |
...
|---|---|---|---|---|---|---|---|---|
| `LineItem` | ✓ | ✓ | ✓ (via phase 4) | N/A | N/A | ✓ | N/A | ✓¹ |
| `ConstructionLine` | ✓ | ✓ | bug² | N/A | N/A | **bug²** | N/A | **bug²** |
| `GridlineItem` | ✓ | ✓ | ✓ (phase 2 + 4) | N/A | N/A | ✓ | N/A | ✓¹ |
| Generic `QGraphicsLineItem` (`Pipe`) | ✓ | ✓ | ✓ (phase 4) | N/A | N/A | ✓ | N/A | ✓¹ |
| `RectangleItem` | ✓ (corners) | ✓ (edge centers) | ✓ (phase 4) | ✓ | N/A | ✓ | N/A | ✓¹ |
| `QGraphicsEllipseItem` (full circle — `CircleItem`) | N/A | N/A | ✓ (phase 4 vs segments) | ✓ | ✓ | ✓ | ✓ | ✓¹ |
| `QGraphicsEllipseItem` (Node) | N/A | N/A | N/A | ✓ | N/A (suppressed) | N/A | N/A | N/A |
| `WallSegment` | **bug³** | ✓ (centerline + face mids) | **bug⁴** | N/A⁵ | N/A | ✓ (5 segments) | N/A | ✓¹ |
| `PolylineItem` | ✓ (vertices) | ✓ (segment mids) | ✓ (phase 4) | N/A | N/A | ✓ | N/A | ✓¹ |
| `ArcItem` | ✓ (start/end) | ✓ (angular) | ✓⁹ (phase 4 vs segments) | ✓ | ✓ (in-range) | ✓ (exact on the arc, in-range) | ✓⁶ | ✓ (exact) |
| `EllipseItem` (rotatable) | N/A | N/A | ✓⁹ (phase 4) | ✓ | ✓ (axis ends, rotation-aware) | ✓¹⁰ | N/A | ✓¹⁰ |
| `SplineItem` | ✓¹¹ (control points) | N/A | ✓⁹ (phase 4) | N/A | N/A | ✓¹⁰ | N/A | ✓¹⁰ |
| Generic `QGraphicsPathItem` (DXF) | ✓ (vertices) | ✓ (segment mids) | ✓ (phase 4) | N/A | N/A | ✓¹⁰ | N/A | ✓⁸ ¹⁰ |
| `TextItem` | ✓ (frame-box corners) | ✓ (frame-box edge mids) | N/A | ✓ (frame-box centre) | N/A | N/A | N/A | N/A |
| `BlockInstance` | ✓ (stroked on-curve vertices + text frame-box corners) | ✓ (text frame-box edge mids) | N/A | ✓ (insertion origin + text frame-box centres) | N/A | N/A | N/A | N/A |

**Notes:**
1. ~~**Nearest is unreachable when perpendicular is on.**~~ **Fixed 2026-04-08** (roadmap item 5). `_geometric_snaps` now emits `perpendicular` and `nearest` as independent candidates; the picker's priority band resolves the winner. Since 2026-09-25 `perpendicular` is emitted only as perpendicular-from (§4), so without a start point `nearest` is the sole foot candidate. Regression test: `tests/test_snap_nearest_perpendicular_decoupling.py`.
2. **ConstructionLine semantics are inconsistent with its visual.** `ConstructionLine` IS a `QGraphicsLineItem` subclass (`geometry_2d.py:54`), so phase-1 `_geometric_snaps` fires its line branch — but via `item.line()`, which returns the *extended drawn* line, not the `pt1`/`pt2` anchors. Phase 4 (`_check_geometry_intersections` line 335) treats it as a **finite** segment between `pt1`/`pt2`, missing intersections outside the anchor range even though the line is conceptually infinite. **Deferred** (roadmap item 4 downgraded to P3): the ConstructionLine two-click tool is effectively unused; revisit if/when the feature sees real usage.
...
6. ~~Tangent is implemented only for full circles.~~ **Stale — arcs do emit tangents.** The `ArcItem` branch of `_geometric_snaps` emits `tangent` candidates (only tangent points that fall on the visible arc) alongside its exact perpendicular/nearest feet; the circle-foot math is shared with the full-circle branch.
7. ~~Generic `QGraphicsPathItem` items (DXF imports) emit endpoints and midpoints in `_collect`, but `_check_geometry_intersections` has no branch for them.~~ **Fixed.** Phase 4 now extracts segments from `QGraphicsPathItem` items (DXF imports). In the main scene, underlay geometry is queried from the `UnderlaySnapIndex` rather than descending into child items. In the import dialog (no snap index), `_phase4_items()` yields invisible child items directly. DXF path items fully participate in phase-4 intersection scans in both contexts. Phase 4 filters each path segment against the search rect before extraction to avoid polyline bounding-box blowup (`_PHASE4_MAX_SEGMENTS = 256` bail-out).
8. **Nearest now works for underlay geometry.** In the main scene, `_query_underlay_snaps()` retrieves geometry from the `UnderlaySnapIndex` and computes perpendicular/nearest snaps against index results (2026-05-22). In the import dialog (no snap index), `_check_scene_items` calls `_geometric_snaps()` on invisible child items directly, enabling nearest and perpendicular snaps on DXF/PDF underlay geometry. The import dialog preview uses invisible individual items (non-cosmetic pen with `line_weight = 1.5 * import_scale`, `NoBrush`) alongside batched QPainterPaths for rendering.
9. **Curve intersections are approximate.** Phase 4 extracts a generic `QGraphicsPathItem`'s segments from its *raw* path elements, so for `ArcItem`, `EllipseItem` and `SplineItem` (all `QGraphicsPathItem` subclasses whose paths are cubic Béziers) the intersection candidates are computed against the Bézier control polygon, not the drawn curve. Unchanged by the snap-polish batch (only the cursor-dependent phase-1 snaps moved to the flattened curve — note 10).
10. **Flattened-curve projection.** Cursor-dependent snaps (perpendicular, nearest) on `EllipseItem`, `SplineItem` and generic paths project onto the path **flattened in scene coordinates** (Bézier control points sit off the curve, so projecting onto raw elements snapped to empty space). Coverage is complete: instead of a segment-count cap, subpaths and segments whose bounds lie farther than the search radius from the cursor are culled. A pathological path — more than `SNAP_MAX_FLAT_POINTS` elements or flattened points — falls back to the bounded raw-element walk (capped at `SNAP_MAX_SEGMENTS_PER_ITEM`). Both caps live in `constants.py`. `ArcItem` and full circles use exact analytic feet instead; circles read their geometric `rect()`, never the bounding rect (a `CircleItem`'s bounding rect is its padded hit stroke, cached at the zoom it was first computed).
11. `SplineItem` endpoints are its **control points** (`grip_points()`), not the tessellated curve; interior control points generally lie off the curve.

**`HatchItem` was retired (2026-08-22)** — fills are now properties of the geometry — so it no longer has a row. It never contributed candidates; the hatch-noise case study (§7.2) was not caused by hatch leakage (see that section).
...
1. **Phase 1 — Scene items in search rect.** Iterates `scene.items(search_rect, IntersectsItemBoundingRect)`. Child items are checked first: children of `DXF Underlay` / `PDF Underlay` groups take one of two paths depending on whether the group carries an `UnderlaySnapIndex` (stored in `data(4)`): **(a) Snap index path (main scene):** the index is queried once per group via `_query_underlay_snaps()`, which maps the search rect into group-local coordinates and retrieves nearby geometry from the spatial index — individual child items are not processed. **(b) Direct child path (import dialog):** when no snap index exists, individual invisible child items are processed directly via `_collect()` and `_geometric_snaps()`, identical to the pre-index behavior. All other child items are skipped. Top-level items skip the origin marker, items above z=150, pipes (when `skip_pipes` is set — design-area mode), and underlay groups themselves (queried through their index instead). There is no item-type skip list: `TextItem` is **not** skipped — it snaps by its frame box (§5). For each surviving top-level item, calls `_collect()` for static snaps and `_geometric_snaps()` for cursor-dependent snaps (with the placement start point, if any, for perpendicular-from — §4; generic paths use the flattened, culled walk of §5 note 10).
...
| 2 | ~~done~~ | Phase-4 segment-source filter audit — **done 2026-05-22**. All item types audited; `QGraphicsPathItem` segments filtered per-segment against search rect; `_PHASE4_MAX_SEGMENTS` bail-out; `IntersectsItemBoundingRect` mode on all `scene.items()` calls | `_check_geometry_intersections` only iterates item types whose §5 row contributes to phase 4 | `[ref:snap-spec§5,§6]` |
...
| 6 | ~~done~~ | `ArcItem` tangent support — **already present** (the item was stale; confirmed 2026-09-24, §5 note 6): the arc branch emits tangent points on the visible arc | `_geometric_snaps` arc branch emits tangent candidates when `snap_tangent` is on; matrix cell `ArcItem×tangent` flips to ✓ | `[ref:snap-spec§5-row-ArcItem]` |
| 7 | ~~done~~ | Generic `QGraphicsPathItem` (DXF) phase-4 segments — **done 2026-05-22**. Phase 4 extracts segments from DXF path items with per-segment spatial filtering against search rect. Main scene queries `UnderlaySnapIndex` for underlay segments; import dialog falls back to processing invisible child items directly (no snap index) | `[ref:snap-spec§5-note-7]` |
...
`SnapEngine.find()` accepts two optional ALIGN arguments: `align_paths` (a list of ALIGN `align_engine.Ray` tracking vectors) and `align_aperture_px` (their separate px grab-radius, default `ALIGN_PATH_TOL_PX`). When `align_paths` is provided, `find()` adds three candidate families to the **same** `_SnapCtx` priority-band picker — path×path and path×geometry crossings as `align_intersection` (priority **20**) and single-path projections as `align_path` (priority **30**) — so they rank **below every real snap** (priorities 0–7), subject to the weak-foot rule below. ALIGN candidates are judged at `align_aperture_px` via a per-candidate `ctx.check(..., aperture_px=…)` override (not the 15px real-snap aperture), and a held ALIGN result is released at that wider aperture. Real-snap acceptance is never affected. The picker/hysteresis (§6.1, §14.2) and px judgment (§14.1) are unchanged — ALIGN is one more candidate source, not a second resolution path.

**One picker at the seam (2026-09-24).** `Model_Space.get_effective_position` makes **one** `find()` call per move that ranks real SNAP and ALIGN candidates together (there is no separate ALIGN tier); the design-area branch keeps its own `only_types={"center"}` call (§14.3). The call also passes `from_point=` — the placement start point that turns `perpendicular` into perpendicular-*from* (§4). Routing of the winner and the `held` rule are owned by [`align-placement.md §3`](align-placement.md#3-lock-guide-one-picker-composition).

**Weak-foot dominance.** The ranking exception that lets an in-aperture `align_intersection` beat a cursor-foot `nearest` / `perpendicular` — while a strong real snap that beats the crossing pairwise is restored — is specified in [`align-placement.md §3.1`](align-placement.md#31-candidate-families-priority). In the picker it is `_SnapCtx.weak_types` (always `{nearest}`, since `perpendicular` exists only as perpendicular-from, §4) plus `_SnapCtx.strong_best` (the best in-aperture non-weak, non-ALIGN candidate), and it is mirrored in the §14.2 hold rule. The three band rules are the shared `_band_beats()` helper.

**Handle-snap results** (`HandleSnapResult`, the S2 move marker — [`selection-manipulator.md`](selection-manipulator.md) "Move — handle snap") are never fed back as `held`: rule owned by [`align-placement.md §3`](align-placement.md#3-lock-guide-one-picker-composition).
