...
| **nearest** | cross | `#aaaaaa` grey | `NEA` | 7 | Closest point to the cursor on a segment / curve (always the cursor foot, clamped onto the segment) — emitted independently of `perpendicular` (§5 note 1) | works |
...
**Weak-foot dominance.** The ranking exception that lets an in-aperture `align_intersection` beat a cursor-foot `nearest` / `perpendicular` — while a strong real snap that beats the crossing pairwise is restored — is specified in [`align-placement.md §3.1`](align-placement.md#31-candidate-families-priority). In the picker it is `_SnapCtx.weak_types` (derived from `from_point`: `{nearest}` with a start point, `{nearest, perpendicular}` without) plus `_SnapCtx.strong_best` (the best in-aperture non-weak, non-ALIGN candidate), and it is mirrored in the §14.2 hold rule. The three band rules are the shared `_band_beats()` helper.
