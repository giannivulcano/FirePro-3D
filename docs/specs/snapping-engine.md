...
**Handle-snap results are never `held`.** While a selection moves, the S2 handle snap publishes its marker into `Model_Space._snap_result` as a `HandleSnapResult` (an `OsnapResult` subclass, `handle_snap.py`; mechanics owned by [`selection-manipulator.md`](selection-manipulator.md)). The seam drops a `HandleSnapResult` instead of feeding it to `find()` as the §14.2 hysteresis `held` result — it is a handle's target, not a cursor snap.
