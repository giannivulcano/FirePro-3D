...
7. ~~Generic `QGraphicsPathItem` items (DXF imports) emit endpoints and midpoints in `_collect`, but `_check_geometry_intersections` has no branch for them.~~ **Fixed.** Phase 4 now extracts segments from `QGraphicsPathItem` items (DXF imports) and `_phase4_items()` descends into underlay groups. DXF path items fully participate in phase-4 intersection scans. Phase 4 filters each path segment against the search rect before extraction to avoid polyline bounding-box blowup (`_PHASE4_MAX_SEGMENTS = 256` bail-out).
8. **Nearest now works for underlay children.** `_check_scene_items` calls `_geometric_snaps()` on underlay group children (2026-05-22), enabling nearest and perpendicular snaps on DXF/PDF underlay geometry. Previously only `_collect()` ran on these items. The import dialog preview uses invisible individual items (transparent cosmetic pen, `NoBrush`) alongside batched QPainterPaths for rendering, so the snap engine processes them identically to plan-view underlays.
...
| 7 | ~~done~~ | Generic `QGraphicsPathItem` (DXF) phase-4 segments — **done 2026-05-22**. Phase 4 extracts segments from DXF path items with per-segment spatial filtering against search rect. Import dialog uses invisible individual items (hybrid architecture) so snap engine processes them identically to plan-view underlays | `[ref:snap-spec§5-note-7]` |
