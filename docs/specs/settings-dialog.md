...
- **Startup:** the initial untitled project is a **clone of the template** (replaces the removed override path).
- **New Project:** clears the scene and **clones the template** again.
- **"Save as default for new projects":** writes the *current project's* `scale` + `project_info` into the template file, preserving its blank geometry (copies **settings only**).
