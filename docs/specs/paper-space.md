...
5. `apply_paper_overrides(source_scene, crop, paper_scale=fit, …, viewport_data=data)`
   ...
   - **Rooms plot as boundary + tag only** — `apply_paper_overrides` flags each
     `Room` with `_paper_no_fill`; `Room.paint` then draws the boundary (and the
     tag renders as its own child item) with **no fill** (a filled room reads as a
     solid blob on a sheet). The model canvas is untouched (keeps the alpha-50 wash
     / opaque `_paper_fill_opaque` fill); the flag is cleaned up on restore.
...
- **Selection wiring:** `PaperScene.selectionChanged`, `undo_stack.indexChanged`, and tab changes all re-populate the panel (main window `update_paper_property_manager`). Selection routing: selected `TextAnnotationItem`s → else a single selected `SheetViewport` → else the active sheet. A selected viewport shows the **`ViewportProperties`** adapter (Title, Scale, Show Border, Position X/Y, read-only derived Width/Height), **superseding `SheetViewPropertiesDialog` for editing** (§19.4); the double-click dialog and the panel share `PaperScene.commit_viewport_edit` so the crop×scale size derivation has one home, and every edit is undoable (`ChangeViewportPropertiesCommand`). The ribbon contextual Viewport tab (the "both panel + ribbon" other half) is a filed follow-up — it needs the contextual-tab machinery (`ribbon-bar.md` D9). **While Add-Text mode is active the panel keeps showing the template** (refreshes must not blank it — e.g. an undo mid-placement fires `indexChanged` with nothing selected).
...
- **Label height ownership:** paper heights are **Display-Manager paper-category settings**, not per-item fields (grill decision 2026-08-07: uniform sizing per category; a per-item override can return if a real need appears). **Room labels adopt this mechanism (2026-08-27):** the "Room" paper category carries `label_height_mm` (default 2.5 mm); `apply_paper_overrides._apply_room_label_paper_height` sizes the label font to `label_height_mm / paper_scale` and `restore_model_display` puts the model-unit size back — so a room tag plots at a constant on-paper cap height instead of shrinking to sub-pixel at plot scale. **Pipe labels remain model-unit sized** (same follow-up).
...
- `SheetViewport.paint` computes the true geometric scale `S = min(vp_rect.width()/source.width(), vp_rect.height()/source.height())` (paper mm per model mm; matches `QGraphicsScene.render`'s KeepAspectRatio behavior under grip-resized aspect divergence, and correct even for NTS viewports where declared scale is 0.0) and passes it to `apply_paper_overrides(scene, rect, paper_scale=S)`.
...
- **Panel precedence (paper tab):** paper-scene selection (single `SheetViewport` → `ViewportProperties`; else `TextAnnotationItem`s) → else browser-selected sheet (`sheetSelected`; ignored while Add-Text mode owns the panel — §9.6) → else **active sheet**. The panel is never blank on the paper tab. Stale-adapter guards: `_delete_sheet`, `_apply_loaded_file`, and `new_file` all reset the panel so an adapter never outlives its sheet across delete/load boundaries.
- **Sheet Properties fields:** Sheet Number, Sheet Name, Paper Size, Orientation, **Rev, Date, Drawn By, Checked By** (the last four are per-sheet `title_block_fields`, routed through `SetSheetFieldCommand` — undoable — and feed the `@[…]` title-block tokens, §8.4), plus the **Edit Revisions…** button.
- **Viewport Properties (`ViewportProperties`):** selecting one viewport populates the panel (Title, Scale, Show Border, Position X/Y, read-only derived Width/Height); edits route through `PaperScene.commit_viewport_edit → ChangeViewportPropertiesCommand`, sharing the double-click dialog's crop×scale size derivation. This supersedes `SheetViewPropertiesDialog` for editing; the ribbon contextual Viewport tab is a filed follow-up (needs the contextual-tab machinery).
