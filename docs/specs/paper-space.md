...
**Date:** 2026-04-09
**Complexity:** Large
**Status:** partial — Phase-1 (sheet/viewports/title block) + the single-sheet plot step + **sheet text annotations (§9) with property-panel editing + persisted Add-Text template (§9.5/§9.6) + the unified paper-space undo/redo stack (§17)** are built. Batch/multi-sheet UI and the remaining annotation types (leader/line/cloud/north-arrow/scale-bar) are pending; viewport properties are slated to move from `SheetViewPropertiesDialog` into the panel (follow-up).
...
### 9.5 Authoring Interactions [revised 2026-07-09 — panel supersedes dialog]

An **"Add Text"** button on the `PaperSpaceWidget` toolbar enters place-mode (and emits `add_text_mode_toggled`, which the main window uses to show the **text template** in the property panel — §9.6); a click on the sheet drops a *transient* item already in inline edit, **seeded from `PaperScene.text_template`** (formatting fields only — never text/position/wrap). The first non-empty commit pushes an `AddTextAnnotation` command; an empty or Esc-cancelled placement discards silently (nothing on the undo stack). Existing blocks: double-click → inline edit; drag → move (anchor clamped to the paper rect); right-click → **Delete only** (no Properties entry — formatting lives in the panel); `Delete` → remove when selected (a character while editing). Event-driven deletes defer via `QTimer.singleShot` (no reentrant destruction), mirroring viewport delete.

### 9.6 Property-Panel Editing [rewritten 2026-07-09 — replaced the modal dialog]

`TextAnnotationPropertiesDialog` is **deleted**. Formatting is edited in the right-side property panel: `TextAnnotationItem` implements the duck-typed `get_properties()`/`set_property()` protocol (rows: font, height, bold, italic, color, alignment, opaque background, plus a read-only **Leader** placeholder for the leader follow-up). Widget/write-path mechanics are owned by `property-panel.md` (§3.2/§3.3 — Rule A); this section owns the text-specific contracts:
...
- **Selection wiring:** `PaperScene.selectionChanged`, `undo_stack.indexChanged`, and tab changes all re-populate the panel (main window `update_paper_property_manager`); only `TextAnnotationItem`s are shown — viewports keep `SheetViewPropertiesDialog` (follow-up filed).
