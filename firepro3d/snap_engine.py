...
class SnapEngine:
    ...
    def _collect(
        ...
        #    quadrants that are wrong for a rotated ellipse) ────────────────
        elif isinstance(item, EllipseItem):
            gp = item.grip_points()   # [centre, major+, major-, minor+, minor-] scene coords
            if self.snap_center:
                pts.append(("center", gp[0], None))
            if self.snap_quadrant:
                for q in gp[1:5]:
                    pts.append(("quadrant", q, None))
