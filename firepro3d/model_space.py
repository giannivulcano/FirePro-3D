...
class Model_Space(HaloSelectionMixin, SceneIOMixin, QGraphicsScene):
    ...
    def set_mode(self, mode, template=None):
        ...
        # 2D-geometry drawing teardown (line/rect/circle/polyline + polygon/arc)
        # — owned by the geometry drawing controller (slices 8+9). Idempotent;
        # the per-primitive "keep if staying in that mode" guards live inside
        # clear(). Text/dimension teardown stays inline below (other concerns).
    ...
    def _hud_available(self) -> bool:
        ...
        return self._plc._hud_available()

    def _create_dynamic_input(self):
        """Shell → PlacementInputCoordinator._create_dynamic_input."""
        return self._plc._create_dynamic_input()

    def _on_dynamic_input_field_committed(self) -> None:
        """Shell → PlacementInputCoordinator._on_dynamic_input_field_committed."""
        return self._plc._on_dynamic_input_field_committed()
    ...
    def _sync_dynamic_input(self) -> None:
        """Shell → PlacementInputCoordinator._sync_dynamic_input."""
        return self._plc._sync_dynamic_input()

    def begin_dynamic_input(self, seed: str = "") -> bool:
        """Shell → PlacementInputCoordinator.begin_dynamic_input."""
        return self._plc.begin_dynamic_input(seed)

    def _visible_view(self):
        """Shell → PlacementInputCoordinator._visible_view."""
        return self._plc._visible_view()
    ...
    def end_dynamic_input(self) -> None:
        ...
        return self._plc.end_dynamic_input()

    def _on_dynamic_input_cancelled(self) -> None:
        """Shell → PlacementInputCoordinator._on_dynamic_input_cancelled."""
        return self._plc._on_dynamic_input_cancelled()

    def _on_dynamic_input_committed(self, values: dict) -> None:
        """Shell → PlacementInputCoordinator._on_dynamic_input_committed."""
        return self._plc._on_dynamic_input_committed(values)
    ...
    def apply_dynamic_input(self, geometry):
        """Shell → PlacementInputCoordinator.apply_dynamic_input."""
        return self._plc.apply_dynamic_input(geometry)
    ...
    def _halo_candidate_ok(self, item, scene_arg):
        """HaloSelectionMixin candidate-filter hook (C1): a labelled Room is a
        candidate only via its label rect; a label-less Room falls back to its
        full polygon (shape() untouched). Everything else is accepted.

        ``scene_arg`` is a QPointF (aperture hover, halo_candidates_at) or a
        QRectF (rubber-band, rubber_band_hits); dispatch to _halo_room_hit
        accordingly to preserve the historical point-vs-rect filter semantics.
        """
        if isinstance(item, Room):
            is_rect = isinstance(scene_arg, QRectF)
            return self._halo_room_hit(item, scene_arg, is_rect=is_rect)
        return True

    def _halo_mode_ok(self):
        """HaloSelectionMixin mode hook: HALO runs only in select/idle mode."""
        return self.mode in (None, "select")
    ...
    def _preview_from_arc(self, resolved) -> None:  # shell → GeometryDrawingController (slice 9)
        return self._geom_ctl._preview_from_arc(resolved)
    ...
    def _move_wall_rect(self, *args, **kwargs):  # shell → WallPlacementController (slice 10, C2)
    ...
    def _commit_draw_arc_rim_at(self, point) -> bool:  # shell → GeometryDrawingController (slice 9)
        return self._geom_ctl._commit_draw_arc_rim_at(point)

    def _arc_end_point_for_span(self, span_deg) -> "QPointF":  # shell → GeometryDrawingController (slice 9)
        return self._geom_ctl._arc_end_point_for_span(span_deg)

    def _apply_arc_dynamic_input(self, geometry) -> bool:  # shell → GeometryDrawingController (slice 9)
        return self._geom_ctl._apply_arc_dynamic_input(geometry)

    def _commit_draw_arc_at(self, end_point) -> bool:  # shell → GeometryDrawingController (slice 9)
        return self._geom_ctl._commit_draw_arc_at(end_point)
    ...
    def _polygon_readout(self) -> str:  # shell → GeometryDrawingController (slice 9)
        return self._geom_ctl._polygon_readout()

    def _press_polygon(self, event, pos, snapped, item_under, node_under, pipe_under):  # shell → GeometryDrawingController (slice 9)
        return self._geom_ctl._press_polygon(event, pos, snapped, item_under, node_under, pipe_under)

    def _polygon_rotation_angle_to(self, cursor) -> float:  # shell → GeometryDrawingController (slice 9)
        return self._geom_ctl._polygon_rotation_angle_to(cursor)

    def _apply_polygon_dynamic_input(self, geometry) -> bool:  # shell → GeometryDrawingController (slice 9)
        return self._geom_ctl._apply_polygon_dynamic_input(geometry)

    def _commit_polygon_at(self, rim):  # shell → GeometryDrawingController (slice 9)
        return self._geom_ctl._commit_polygon_at(rim)

    def _preview_from_polygon(self, tip):  # shell → GeometryDrawingController (slice 9)
        return self._geom_ctl._preview_from_polygon(tip)

    def _preview_polygon_rotation(self, angle_deg) -> None:  # shell → GeometryDrawingController (slice 9); called by PlacementInputCoordinator
        return self._geom_ctl._preview_polygon_rotation(angle_deg)

    def _cycle_polygon_sides(self, direction: int):  # shell → GeometryDrawingController (slice 9)
        return self._geom_ctl._cycle_polygon_sides(direction)

    def _toggle_polygon_inscribed(self):  # shell → GeometryDrawingController (slice 9)
    ...
    def _press_opening(self, *args, **kwargs):  # shell → FeaturePlacementController (slice 11, C1)
        return self._feature_ctl._press_opening(*args, **kwargs)

    def _press_door(self, *args, **kwargs):  # shell → FeaturePlacementController (slice 11, C1)
        return self._feature_ctl._press_door(*args, **kwargs)

    # ── Window placement (legacy shim) ────────────────────────────────
    def _press_window(self, *args, **kwargs):  # shell → FeaturePlacementController (slice 11, C1)
