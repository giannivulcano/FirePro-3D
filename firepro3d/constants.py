...
# ── Selection-manipulator handle style (docs/specs/selection-manipulator.md) ──
# Mockup-approved 2026-08-30 (style A): dark-filled square handles with a
# theme-``selection`` border. The FILL is a function of
...
MANIP_HANDLE_BORDER_PX = 1.2               # handle outline width (device px)
...
HALO_APERTURE_PX = 15     # screen-space pick aperture radius (px)
# SNAP-style priority band (px): candidates within this px distance of the
# closest hit rank by runtime-Z desc, then distance (selection-mode.md §4.1).
HALO_PRIORITY_BAND_PX = 12
...
SELDIM_LABEL_OFFSET_PX = 6       # every linear label sits this far off its line (smoke: 14 -> 10 -> 6)
...
SELDIM_ARROW_PX = 7              # reference-arc arrowhead length
SELDIM_ARROW_HALF_W_PX = 2.5     # reference-arc arrowhead half-width
