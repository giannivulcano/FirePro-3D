...
- [ ] [type:bug] Update the C1-gated loose-authoring tests to construct a block-editor scene [P2] [subject:CAD]
  - Details: pre-existing since the C1/C8 landing (A/B-proven identical on `4906ffe`): ~23 tests author loose 2D geometry in a plain `Model_Space()` (plan role) via `set_mode("draw_*")`, which C1 now refuses (`authoring_allowed` gate) → `scene.mode` stays `select`. Affected: `test_tool_shortcuts` (bare-key draw modes), `test_polygon_placement`, `test_polyline_close_placement`, `test_ellipse_spline_draw`, `test_align_universal_scope` (align in draw modes), `test_dynamic_input_schema`/`_seam`. Fix: construct `Model_Space(scene_role="block_editor")` (the surface where loose authoring is legal) or drive placement through the Block-Editor scene. Not caused by C3 — surfaced as pre-existing during the C3 gate. (`test_design_area::TestPickMode::test_design_area_mode_skips_grips`, `test_graphic_override` floor-tab, `test_icon_theming` geom2d icons are separate pre-existing failures — triage individually.)
