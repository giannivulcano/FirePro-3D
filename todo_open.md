...
- [ ] [type:maint] SPIKE (blocking): bench batched-render-from-native-primitives vs today's hand-batched underlay path on a real large DXF [P1] [subject:CAD]
...
- [ ] [type:feature] Implement the reference-graphic unification (Underlay = special-case Block) [P1] [subject:Architecture]
...
- [ ] [type:feature] Implement the containment contract — model placement-only + Text primitive + ribbon rework + clean-drop load [P1] [subject:Architecture]
  ...
  - **Includes the per-spec FULL body rewrites + frontmatter re-stamp** (the deferred half of the per-spec reconciliation): the 2026-09-16 pointer-annotation stage landed `related-contract:` keys + banners + C-invariant section pointers on all 6 specs (as-built bodies left accurate) — the full rewrites (bodies now describing target, `status`/`last-verified`/`verified-commit` re-stamp) bind HERE, when the code actually changes. Specs: `2d-geometry.md`, `view-relationships.md`, `ribbon-bar.md` (D10), `block-system.md`, `model-space-architecture.md`, `underlay-workflow.md`. Close the contract's `[ ] Per-spec full body rewrites…` verification box on landing.
