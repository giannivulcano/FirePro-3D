...
- [ ] [type:feature] Implement the containment contract — remaining: C3 level-on-instance + per-spec doc rewrites [P1] [subject:Architecture]
  - Details: on `feat/containment-contract`. **LANDED (slices in todo_closed.md):** C1 authoring gate (`Model_Space.set_mode` refuses the 8 loose primitives + text + dimension via scene role) + C8 silent clean-drop (both serialization paths shed loose geometry/text/dimension/constraints; legacy paths deleted); C5 unified `TextItem` (replaces `NoteAnnotation` + `TextAnnotationItem`, compiles to outlined glyphs); C7 ribbon rework (Create dissolved → 5-tab roster; Architecture Block + Underlay groups; Quick/Text-Block retired; Text in the Block-Editor palette — commit 8c887aa). **REMAINING:** (C3) make the 8 non-text 2D primitives fully definition-local/level-less and move level scope onto the Block instance (`TextItem` already dropped level; the others still carry `level`/`_level_offset_mm`/`Z_CAT_CONSTRUCTION`). `geometry_2d.py`, `block_instance.py`, `elevation_scene.py`. ref: model-space-containment-contract §C3 / D3.
  - **Remaining per-spec FULL body rewrites + frontmatter re-stamp** (the deferred half of the per-spec reconciliation): DONE — `scene-io.md` (forged/stamped, C1/C8) + `ribbon-bar.md` (§3.4 roster + D10 resolved + stamped, C7, 2026-09-18). REMAINING — `2d-geometry.md` (definition-local/level-less primitives + Text as 9th, C1/C3/C5), `view-relationships.md` (§3.1 compose-by-placement, C1), `block-system.md` ("Feature composes Blocks" + Quick-Block retirement, C2/C7), `model-space-architecture.md` (placement-only + deleted loose paths, C1/C8), `underlay-workflow.md` (ribbon-home move, C7). Then close the contract's `[ ] Per-spec full body rewrites…` box. (Do alongside C3, or as a standalone doc-reconciliation pass — several specs still describe as-built while the code has moved.)
...
- [ ] [type:bug] 12 pre-existing test failures (placement continuous-mode + dynamic-input schema + geom2d icons) [P2] [subject:Testing]
  - Details: found 2026-09-17 during the reference-graphic wrap-up broad-suite run; **PROVEN pre-existing** (identical 12 fail at branch point `0a634fe` with that work absent — NOT the reference-graphic change). Deterministic in isolation (not environmental). Failures: `test_polygon_placement.py::{test_three_step_state_cleared_after_commit,test_continuous_placement_after_commit}`, `test_polyline_close_placement.py::{test_click_start_closes_with_3_points,test_close_via_real_clicks}` (assert `scene.mode == "polyline"` after a close, got `"select"` — looks like the single-placement `_end_placement_switch` now fires for polygon/polyline close, contradicting these continuous-mode tests), `test_ellipse_spline_draw.py::test_three_click_ellipse_placement`, `test_dynamic_input_schema.py::TestRegistry::{test_schemas_registered,test_placement_flag_matches_resolve_return_type,test_requires_anchor_covers_placements_plus_move}`, `test_dynamic_input_seam.py::TestCommitAndCancel::test_continuous_commit_closes_cleanly`, `test_design_area.py::TestPickMode::test_design_area_mode_skips_grips`, `test_graphic_override.py::test_floor_tab_has_edit_and_graphic_override_groups`, `test_icon_theming.py::test_geom2d_icons_exist_and_are_two_token_compliant`. Likely fallout of a recent placement/ribbon/icon change; triage whether the tests or the code are stale (the polyline/polygon ones may just need updating for the single-placement close behavior). `model_space.py`, `dynamic_input.py`, `tests/`.
