...
- [ ] [type:maint] Reconcile 5 governing specs to the chrome revamp (Account) [P2] [subject:Documentation]
  - Details: `mainwindow-chrome-revamp.md` is flipped to `status: current` (verified-commit 76757eb) with an as-built deviations note, but the per-spec bodies still describe pre-revamp behavior. Reconcile in place (pointer-first per the consolidating-contract model): `ui-design-system.md` (flip MainWindow re-shell wave #2 deferred→current; header/footer-rail invariants + new `M.FOOTER_*` tokens), `ribbon-bar.md` §3.4 (roster File/Settings/Display, TopTabs accent-underline + green hover, vertical ALL-CAPS labels, button icon48/height68/stack88, File 3-stack+Recent), `snap-toolbar.md` (`_SnapToolbar` retired → footer InlineOsnapBar; Snap-Settings via footer SNAP-pill), `settings-dialog.md` (ribbon Snap/Angle entry points removed; angle-snap in UX pane; `select_pane`), `icon-style-guide.md` (new File-group+header two-token icons). Update `SPEC-INDEX.md` if boundaries moved. (risk:low, effort:M)
