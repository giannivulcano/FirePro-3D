...
> Batch 1 (placement selection, centre-rect HUD, Ctrl-resize, Shift+handle, block-editor ghost dot, plan-view crosshair) shipped 2026-09-16 (`fix/2d-geo-placement-batch1`, see todo_closed.md). These are the remaining items from the same user list.

- [ ] [type:feature] Reference-line standard across 2D geometry (selection-time + ghost weight) [P2] [subject:CAD]
  - Details: adopt the canonical ellipse ref-line style (`_make_ref_line` = width-1 dashed cosmetic, geometry colour; `geometry_2d.py:EllipseItem.paint`) everywhere. (a) Rect: draw selection-time reference lines connecting corners (diagonals) — RectangleItem draws nothing at selection today; (b) Circle: draw a radius reference line + a bounding box at selection — CircleItem draws nothing today; (c) reconcile the ghost-preview weight: shape previews use width-2 DashLine while Line uses width-1 (`geometry_drawing_controller.py`) — pick one standard (width-1). Ellipse/Polygon/Spline already draw selection ref geometry in this style. No picture needed (reuse ellipse code). `geometry_2d.py`, `geometry_drawing_controller.py`.
- [ ] [type:feature] Add a finite construction/reference line as a Line-tool ←/→ variant [P2] [subject:CAD]
  - Details: user wants a thin finite reference line (same width-1 dashed ref-line style) with real start/end points, cycled with ←/→ alongside the Line variant. ⚠️ Reconcile with the stale "retire construction lines" item below: the old infinite `ConstructionLine` xline was ALREADY removed (`scene_io.py` silently drops the legacy `construction_lines` key; no `ConstructionLine` class exists), so this is net-new, not the xline. Needs a short grill (finite-segment semantics, cycle-key contention with future Line/Polyline merge, `.fpd` load path, does it participate in snap/hosting). `geometry_2d.py`, `model_space.py`, `placement_input_coordinator.py`.
- [ ] [type:feature] 2D-geometry contextual ribbon redesign (per the 2026-09-16 [Image #3]) [P2] [subject:UX]
  - Details: the `geo2d` contextual tab is too wide/sparse. Changes: (1) DROP the Placement group (Level/Level Offset — redundant with the property panel); (2) rename the Fill group → **Graphic Override** and condense it (narrower layout); (3) reorder groups **Edit → Constraints (new) → Graphic Override**. Currently `_build_geo2d_context` = Placement + Fill + Edit (`main.py`); the Graphic-override group builder (`_build_graphic_override_group`) exists but is wired to Floor only. Overlaps the filed "populate contextual tabs per-entity" item. Serve an interactive mockup first (house style). `main.py`, `ribbon_bar.py`. ref: ribbon-bar-spec.
- [ ] [type:feature] Model-space text/note two-click placement (mirror rectangle) [P2] [subject:UX]
...
- [ ] [type:maint] Retire construction lines in favour of alignment guides [P3] [subject:Architecture]
  - Details: ⚠️ **LIKELY STALE (verify 2026-09-16):** the `ConstructionLine` xline appears already removed — no `ConstructionLine` class exists and `scene_io.py` silently drops the legacy `construction_lines` key on load. This item's "wired into 11 modules" narrative predates that removal. Confirm nothing lingers (grep `construction_line`), then close — OR reframe as the load-path decision for old `.fpd` files that still hold `{"type": "construction_line"}` records. Note: the user's NEW ask is a *finite* construction/reference line as a Line variant (see the 2D-geometry placement polish section) — the opposite of retiring, and net-new. Original (2026-08-16): xlines set direction only; the inference engine + OSNAP now cover alignment references without a persistent object (Revit reference-plane model). ref: inferred-dimension-driven-placement.
