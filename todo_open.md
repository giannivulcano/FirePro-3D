...
- [ ] [type:maint] SPIKE (blocking): bench batched-render-from-native-primitives vs today's hand-batched underlay path on a real large DXF [P2] [subject:CAD]
  - Details: gates the reference-graphic unification (`reference-graphic-model.md` R4). A/B on a representative large DXF: (1) today's hand-batched cosmetic path (baseline), (2) native primitives as individual cosmetic scene items, (3) native primitives batched per-layer into cosmetic paths at render time (the target). Accept the unified native-primitive model (target Z) only if (3) ≈ (1) at the zoom/pan bar (`underlay-workflow §18`); else ship the dual-representation fallback (Y). Bench first, per `feedback_performance_priority`. `underlay.py`, `underlay_cache.py`, `model_space.py`. ref: reference-graphic-model §R4/Performance.
- [ ] [type:feature] Implement the reference-graphic unification (Underlay = special-case Block) [P2] [subject:Architecture]
  - ⚠️ BLOCKED BY the R4 perf spike above — do NOT start until the spike has run and chosen target (Z) or fallback (Y). The chosen result determines the rendering architecture.
  - Details: post-spike. Unified import front-end → native primitives + layer metadata → definition (embedded one-off or library); model instances = locked references (no primitive edit, C1); Block/Underlay = capability bundle (Underlay adds lightweight render, per-layer/per-primitive visibility, multi-level); selectable-as-a-unit but de-prioritized + "lock in place" toggle (needs selection-mode ranking); migrate existing underlays (re-extract; missing source → cached fallback). Design done — grill in this session's transcript. `underlay.py`, `block_definition.py`, `block_instance.py`, `model_space.py`, `underlay_import_dialog.py`, `selection-mode`. ref: reference-graphic-model.md.
- [ ] [type:maint] Per-spec rewrites to converge on the containment contract (6 specs) [P2] [subject:Documentation]
  - Details: apply the reconciliation-map deltas (contract §"Reconciliation map"): `2d-geometry.md` (primitives = definition-local/level-less; add Text; drop model placement), `block-system.md` (replace "siblings" with "composes Blocks"; Quick Block retirement; standalone-Block in model+paper), `ribbon-bar.md` (dissolve Create; Architecture Block group; retire Text/Quick-Block buttons), `underlay-workflow.md` (forward-pointer to C4), `model-space-architecture.md` (record C1 + loose-path deletion), `view-relationships.md` (§3.1 authoring = place-definitions; supersede 2D-geo level-scope rows). Re-stamp each frontmatter on landing. `docs/specs/*.md`.
- [ ] [type:feature] Implement the containment contract — model placement-only + Text primitive + ribbon rework + clean-drop load [P2] [subject:Architecture]
  - Details: the code migration. Remove loose-geometry authoring + standalone text from Model Space (C1/C8, delete legacy paths); add **Text** as the 9th 2D primitive with a unified data model shared by paper annotation (C5); make 2D primitives definition-local/level-less and move level scope onto the Block instance (C3); ribbon rework — dissolve Create tab, Architecture Block group, Underlay→Architecture, retire Quick/Text-Block buttons (C7). Large; needs its own plan (likely several slices) + a feature branch. `model_space.py`, `geometry_2d.py`, `block_definition.py`, `block_instance.py`, `paper_space.py`, `annotations.py`, `scene_io.py`, `main.py`, `ribbon_bar.py`. ref: model-space-containment-contract.
- [ ] [type:design] Settle Feature hierarchy naming before Phase B (Category→Type→FeatureDef vs Revit Category→Family→Type) [P3] [subject:Architecture]
