...
- [ ] [type:feature] Implement the reference-graphic unification (Underlay = special-case Block) [P1] [subject:Architecture]
  - ✅ UNBLOCKED 2026-09-17 — the R4 perf spike ran (`tools/perf_probe_reference_render.py`, Sleeman 437k-geom DXF) and **chose target (Z)**: batched-render-from-native-primitives holds full zoom/pan/SNAP/ALIGN parity with today (snap 0.24–0.31 ms vs 0.27–0.58 ms baseline; individual scene items died at 11–18 s/snap). **Binding rendering constraints** (see `reference-graphic-model.md` Performance → Spike result): (1) render MUST be batched per-layer cosmetic paths, never per-primitive scene items; (2) snap MUST run off the `UnderlaySnapIndex`/equivalent spatial index, never a scene walk (the scene's `NoIndex` makes per-item snap fatal); (3) native primitives are the definition-side data model — do NOT hold them as scene items in plan views; materialize lazily (Block Editor) to keep resident memory ≈ baseline.
