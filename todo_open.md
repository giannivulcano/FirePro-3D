...
- [ ] [type:bug] Reconcile `test_dynamic_input_schema::TestRegistry` to the `rectangle_center` schema [P2] [subject:CAD]
  - Details: surfaced 2026-09-18 while triaging the C1-gated test cleanup (these are NOT C1-related — they take no scene). The `rect_center_hud` feature added a `rectangle_center` entry to `SCHEMAS` (`dynamic_input.py`) but three registry tests in `tests/test_dynamic_input_schema.py::TestRegistry` were never updated: `test_schemas_registered` (expected set omits `rectangle_center`), `test_placement_flag_matches_resolve_return_type` (`resolve_rectangle_center` errors on the shared sample), `test_requires_anchor_covers_placements_plus_move` (need-set omits `rectangle_center`). Fix: update the three expected sets / resolve invocation to include `rectangle_center` (or confirm it should be excluded). ref: dynamic-input schema registry, `test-harness.md`.
