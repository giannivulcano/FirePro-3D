...
- [ ] [type:feature] Inline double-click text edit live in the scene (model / Block-Editor) [P1] [subject:UX]
  - Details: user, 2026-09-22 (text-panel smoke). Double-click a `TextItem` on the model / Block-Editor canvas to enter edit mode and type **live inline in the scene** (not just via the panel Content box). Blocked by the known live-only caret/paint bug: while a focused `QGraphicsTextItem` blinks its cursor, Qt calls `paint` with an engine-less viewport device so the live text + caret don't render (`project_qpainter_engineless_qgraphicstextitem`; sibling model-note bug in "UI follow-ups" §L75-76). The display fix (todo #62) renders glyphs via `_glyph_outline_local` fill, but the EDIT caret + live keystroke echo still need a real root-cause (QGraphicsView viewport update mode / text-control repaint path) — its own grill→systematic-debugging on the running app. Wire `begin_edit()`/`TextEditorInteraction` to a double-click on the model surface once the caret renders. `firepro3d/text_item.py` (`begin_edit`/`paint`), `firepro3d/model_view.py`, `firepro3d/model_space.py`. ref: text-annotation-system, selection-manipulator.
- [ ] [type:feature] Custom house-styled colour picker dialog widget (with built-in "No Fill") [P1] [subject:UX]
  - Details: user, 2026-09-22 (text-panel smoke). The app uses the stock native `QColorDialog` everywhere (`display_manager`, `roof_dialog`, `titleblock_editor`, `font_group`, `property_manager`/`ui_kit.Swatch`, …). It cannot host a "No Fill" (transparent) option, so the text Fill Color "No Fill" was reverted to a **0%-opacity stopgap** (fill_color set + `fill_opacity=0`). Build a custom, theme-tokenized colour-picker dialog/popup (parity with the custom `Selector`/`Stepper`/`Swatch` panel inputs — native chrome fought styling) with a first-class **No Fill** choice inside the selector, then repoint `ui_kit.Swatch` (and ideally the app-wide `QColorDialog.getColor` call-sites) at it. Restores a true `fill_color=""` no-fill state for text (paint already honours empty `fill_color`). `firepro3d/ui_kit.py` (Swatch + new picker), `firepro3d/property_manager.py`, and the `QColorDialog` call-sites above. ref: text-annotation-system.
