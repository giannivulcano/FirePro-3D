...
- [ ] [type:design] Underlay reference-graphic unification — dedicated design session (contract C4, deferred) [P2] [subject:Architecture]
  - Details: decide whether Underlay becomes a *sibling* of Block under a common "placeable reference graphic" umbrella (shared import + placement/transform UX + library-browse) while keeping distinct rendering/selection/level, OR stays fully separate. A literal "Underlay = BlockInstance" breaks ~9 invariants (flyweight vs batched cosmetic per-layer paths; selectable vs non-selectable; single vs multi-level; per-record snap/cache/colour). Own grill→design. `underlay.py`, `block_definition.py`, `block_instance.py`, `model_space.py`. ref: model-space-containment-contract §C4, underlay-workflow, block-system.
