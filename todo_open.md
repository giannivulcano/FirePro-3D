...
> Batch 1 (placement selection, centre-rect HUD, Ctrl-resize, Shift+handle, block-editor ghost dot, plan-view crosshair) shipped on `fix/2d-geo-placement-batch1`; Batch 2 (reference-line standard C, two-click text F2, contextual-ribbon redesign E, icon family K, line/polyline ghost fix) shipped on `feat/2d-geo-polish-batch2` — both in todo_closed.md. Only D remains from the user list.

- [ ] [type:feature] Add a finite construction/reference line as a Line-tool ←/→ variant [P2] [subject:CAD]
  - Details: **GRILL DONE 2026-09-16 — spec locked.** Non-printing finite reference line (dashed width-1 ref style). Per-item `printed` toggle (default False, `ui_kit.ToggleSwitch` in the property panel per the style guide): not-printed → excluded from paper export AND from the saved block definition; printed → plots dashed + included in the block. First-class persisted item through ALL paths (scene_io + undo capture + block-embed-when-printed); own **"Reference Lines"** Display category; level-scoped; full SNAP participation (always); edits/selects/deletes like a normal line; plan-only (no elevation/3D). Placement: a ←/→ variant on the **Line** tool (single 2-click segment); app-wide. Integration map from the reuse sweep covers geometry_2d (ReferenceLineItem mirroring LineItem), `_reference_lines` list + ~10 collectors, the 3 serialization paths, paper-export exclusion, Display category, snap `_collect`, and a new ToggleSwitch property-panel field type. Renders the "retire construction lines" item below fully obsolete (close on land). Building on `feat/reference-line`. `geometry_2d.py`, `model_space.py`, `placement_input_coordinator.py`, `geometry_drawing_controller.py`, `scene_io.py`, `block_editor.py`, `paper_display.py`, `display_manager.py`, `snap_engine.py`, `property_manager.py`, `level_manager.py`, `scene_tools.py`.
