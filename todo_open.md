...
- [ ] [type:maint] U3 — migrate items onto `manip_handles`, one per PR [P1] [subject:Architecture]
  - Details: framework + CircleItem LANDED 2026-09-08 on `feat/u3-griphandle-circleitem`: live-apply `GripHandle` (`manip_handle.py`) calling each item's `apply_grip`; the `_begin_handle`/`_begin`/`hit_test`-pool/`_reflow_live` manipulator fixes; the `_item_uses_manip_handles` coexistence gate (drawForeground + `_find_grip_hit` skip migrated items); snap-parity via the scene's `get_effective_position` flag-borrow; four per-item semantics admitted as extension points. **PolylineItem + shared `default_grip_handles()` helper LANDED 2026-09-09 on `feat/u3-griphandle-polyline`** (helper is the common body of every `manip_handles()`; CircleItem refactored onto it; Polyline = round vertex grips, no special semantics). **SplineItem LANDED 2026-09-09 on `feat/u3-griphandle-spline`** (round control-point grips, same shape as Polyline, no special semantics). **LineItem LANDED 2026-09-09 on `feat/u3-griphandle-line`** (first with per-item semantics: endpoints round + Ctrl-angle-constrained vs the opposite endpoint via a reusable `EndpointGripHandle(GripHandle)._transform_point`→scene `_constrain_angle`; midpoint round (a move grip — translates the whole line; changed square→round 2026-09-10 per user). Also fixed a framework bug the migration surfaced: `GripHandle.on_cancel` restored EVERY grip, corrupting index-dependent grips like the line midpoint → now restores only the dragged grip. `test_scene_tools` generic `_find_grip_hit` tests moved to a migration-agnostic `_GripStub`). **ArcItem LANDED 2026-09-10 on `feat/u3-griphandle-arc`** (zero special semantics — the legacy grip path explicitly excludes arc from Ctrl-constrain; `default_grip_handles(self, circular={0,1,2})`, all-round grips: centre = move, start/end = geometric endpoints; same shape as Spline/Polyline). **RegularPolygonItem LANDED 2026-09-10 on `feat/u3-griphandle-polygon`** (zero special semantics — legacy grip path excludes polygon from Ctrl-constrain; `default_grip_handles(self, circular=all indices)`, all-round grips: centre = move, N vertices = defining points [vertex drag resizes + rotates via `apply_grip`], handle count tracks `_sides`; same shape as Arc). **EllipseItem LANDED 2026-09-10 on `feat/u3-griphandle-ellipse`** (mirrors CircleItem — an ellipse is a generalized circle: `default_grip_handles(self, circular={0})`, centre round move grip + 4 axis-endpoint sizing grips square [major rx=1,2 also rotates; minor ry=3,4], the square grips rotated to the ellipse orientation via a new opt-in `grip_render_angle(index)` GripHandle hook so they stay radially aligned at placement angle + after rotate, and LIVE during the rotate-knob held-preview via `SelectionManipulator._preview_rotation_deg()` [2026-09-10 smoke follow-ups]; zero special semantics, not box-native). **RectangleItem LANDED 2026-09-10 on `feat/u3-griphandle-rect`** (box-native SPECIAL: rect now provides `manip_handles()` [9 grips] so `_item_uses_manip_handles` is the SINGLE legacy-path skip — the separate `provides_handles_for` skip removed from `drawForeground`/`_find_grip_hit`; `_active_handles` returns the rigid RESIZE set for the box-native unrotated rect [no double-up], a ROTATED rect surfaces its parametric grips [live-apply, local-frame resize via `apply_grip`] REPLACING the legacy green grips — resolves the "Rotated rect shows legacy green grips" todo; `grip_render_angle`=`_angle` aligns the square midpoint grips; `provides_handles_for` kept internal for `_active_handles`/`_frame_is_redundant`). Grip-shape house rule (2026-09-09 smoke; refined 2026-09-10): vertex/endpoint + centre/move grips render round (disc); only inert/derived convenience grips stay square — a midpoint that MOVES the whole item is a move grip → round (LineItem midpoint). Function, not position. Each item passes its round indices via `default_grip_handles(circular=…)`. As-built in `docs/specs/selection-manipulator.md §"U3 — GripHandle"`. Each remaining item exposes its parametric points as `GripHandle`s whose drag calls its existing `apply_grip`, carrying the per-item drag semantics that live in `model_space` today: Ctrl angle-constrain (wall/gridline endpoints — reuse `EndpointGripHandle`, opp 1↔0) via `_transform_point`; gridline multi-select parallel-delta + wall-endpoint propagation via `_after_apply`; the constraint-solver pass (always run). Parity-test each item (posted-event drag == legacy grip drag). **Rectangle is NOT a parametric migration (investigated 2026-09-09): it is box-native** — `manip_capabilities()` includes `"scale"` (unrotated), so `provides_handles_for` already routes a single rect to the manipulator's rigid RESIZE handles and both legacy paths skip it; giving it a plain `manip_handles()` would double up (8 resize + 9 grips). Its handle set IS the rigid resize set via `_active_handles()`'s fallback. **LANDED 2026-09-10** as a box-native SPECIAL migration (see RectangleItem LANDED note above): rect provides `manip_handles()` so `_item_uses_manip_handles` is the single legacy-path skip; unrotated → rigid resize handles, rotated → parametric grips (replaced the legacy green grips). **WallSegment LANDED 2026-09-10 on `feat/u3-griphandle-wall`** (FIRST sibling-mutating migration: `manip_handles()` = [`WallEndpointGripHandle(0,opp=1)`, `WallEndpointGripHandle(1,opp=0)`, `GripHandle(2 round move)`, `GripHandle(3 square width)`]; `WallEndpointGripHandle(EndpointGripHandle)` adds proximity **propagation** to joined walls via `_after_apply`→`scene._propagate_wall_endpoint` [per-frame old-pt capture in `_transform_point`] + **atomic Esc-restore of siblings** via `_extra_snapshots`/`_restore_extra`→`scene._snapshot_wall_endpoints`/`_restore_wall_endpoints` [homed in `WallPlacementController` + thin `Model_Space` bridges]; endpoints Ctrl-constrain [opp 1↔0]; square width grip aligned via `grip_render_angle`=centerline Y-up angle [smoke fix]; rotate knob coexists like Ellipse/Arc, not box-native; 20 parity tests + legacy tests kept green). **GridlineItem LANDED 2026-09-10 on `feat/u3-griphandle-gridline`** (SECOND sibling-mutating migration; the sibling relation is multi-select **parallel-delta**, not coincidence-propagation): `manip_handles()` = 4 `GridlineGripHandle(EndpointGripHandle)`s — endpoints (0,1) round + Ctrl-constrain [opp 1↔0], bubble-standoff grips (2,3) round [draggable move-like affordances, round per smoke feedback]; `apply_grip` edit math (endpoints slide along-axis, bubbles slide standoff) unchanged. Parallel-delta on ALL four grips via `_after_apply`→`scene._propagate_gridline_grip` (bubble standoffs propagate too; exact for parallel selections, under-applies for non-parallel — historical); atomic sibling Esc-restore via `_extra_snapshots`/`_restore_extra`→`scene._snapshot_gridline_grips`/`_restore_gridline_grips` (homed directly in `Model_Space` — no gridline controller yet; legacy `_drag_grip_to` branch refactored to call the shared method). The legacy `_PullTabGrip` child items REMOVED (grips manipulator-owned; also fixed a latent `paper_display.py` AttributeError that poked `gl._grip1/_grip2` in the paper-render pass); `_LockIndicator` stays + nudges `manipulator.rebake()` on lock-toggle. Rotate knob coexists (not box-native). Hover grip-preview dropped (U5). 17 parity/integration tests + existing gridline suite (398-test gate) green. As-built in `selection-manipulator.md §"U3 — GripHandle"` + `grid-system.md §4.3/§5.2/§5.7`. **Room LANDED 2026-09-10 on `feat/u3-griphandle-room`** (SIMPLEST item — a SINGLE conditional label-move grip): `manip_handles()` = `default_grip_handles(self, circular={0})`, one round grip at the label centre; `apply_grip(0)` moves the label; zero special semantics (no Ctrl-constrain, no propagation; boundary vertices are wall-derived, NOT grips). FIRST item to exercise the **state-dependent empty `manip_handles()`** path — a label-hidden room returns `[]` (grip_points empty), so the gate reads "not migrated" but nothing renders on either path (benign; the noted follow-up nit is resolved-by-being-harmless, not a bug). No `_PullTabGrip`/`paper_display`/legacy-test changes (Room drew grips via drawForeground). `MANIP_NO_SOLO_ROTATE` → no solo rotate knob. 6 parity tests. As-built in `selection-manipulator.md §"U3 — GripHandle"`. **DesignArea LANDED 2026-09-10 on `feat/u3-griphandle-designarea`** (Room twin — SINGLE conditional badge-move grip): `manip_handles()` = `default_grip_handles(self, circular={0})`, one round grip at the badge centre; `apply_grip(0)` moves the badge; conditional (badge visible only for a confirmed area with members, per `_sync_badge`) → empty manip_handles when hidden, gate off; zero special semantics. Caps `{translate,rotate}` (badge = fixed-layout table, never scalable); frame wraps the badge box via `manip_bounds`. No `_PullTabGrip`/`paper_display`/legacy-test changes. 6 parity tests. As-built in `selection-manipulator.md §"U3 — GripHandle"`. **FloorSlab LANDED 2026-09-10 on `feat/u3-griphandle-floor`** (PolylineItem twin — polygon boundary vertices): `manip_handles()` = `default_grip_handles(self, circular=all vertex indices)`, one round grip per boundary vertex; `apply_grip`/`grip_points` (move vertex + `_rebuild_path`) unchanged; zero special semantics (polygons excluded from the legacy Ctrl-constrain block; floor boundary not grip-coupled to neighbours → no `EndpointGripHandle`/propagation). Rotate knob coexists (`manip_rotate`); not box-native (no `manip_scale`). 5 parity tests mirroring the polyline file. As-built in `selection-manipulator.md §"U3 — GripHandle"`. **RoofItem LANDED 2026-09-10 on `feat/u3-griphandle-roof`** (FloorSlab twin — identical polygon-boundary-vertex migration; `apply_grip`→`_rebuild_path` regenerates overhang + ridge; zero special semantics; 5 parity tests). **DimensionAnnotation LANDED 2026-09-10 on `feat/u3-griphandle-dimension`** (single round offset grip via `default_grip_handles(circular={0})` — the reposition affordance, round per the gridline bubble-standoff precedent; `apply_grip(0)` changes perpendicular offset unchanged; zero special semantics; caps `{translate}`, interior-drag move via `manip_translate`. Grip now manipulator-owned (was legacy `drawForeground`); updated `test_manip_badge_annotations` to assert the single grip shows with no resize/rotate handles. 5 parity tests). Remaining (simplest-first, one PR each): **Text blocks — both BOUNDING-BOX-governed (box-native like Rectangle: frame + resize + move + rotate, NOT a single MText position grip)**: (a) 2D-geometry MTEXT text block `NoteAnnotation` (model scene; has a ribbon button; TODAY translate-only, single position grip → must be UPGRADED to box-native: add `manip_scale`/`manip_rotate` + bounding box; decide resize semantics [wrap-width vs font scale]); (b) paper **Sheet Text block** `TextAnnotationItem` (already box-native: caps translate+scale, manipulator resize handles; paper-scene, folds with U5/paper handle work) · elevation/view-marker items. DetailMarker DONE 2026-09-11 on feat/detail-marker-rect-crop as its own FEATURE: a PARAMETRIC editable crop (8 square crop grips + round bubble grip, live apply_grip resize, manipulator frame = visible box, axis-aligned) + two general detail-view seams (_painting_into_clip_view suppresses dimmed chrome under the crop mask; SelectionManipulator.render_overlay redraws frame+handles+callout BRIGHT after the mask so the crop is editable from the detail view; passive blue box removed). Box-native tried+reverted. As-built in selection-manipulator.md. Follow-up nit: `_item_uses_manip_handles` treats an empty `manip_handles()` as "not migrated" (unreachable for CircleItem; handle when a state-dependent-hittability item lands). per-item modules, `model_space.py`. ref: selection-manipulator §Unification.
