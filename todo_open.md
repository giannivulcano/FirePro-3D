...
- [ ] [type:feature] On-selection dimension readouts for 2D primitives (linear + angular, live, HALO-pickable, click-to-type) [P2] [subject:CAD]
  - Details: user, 2026-09-24. A painted dimension label (Value font `FONT_VALUE`, TextItem-style box) over the selected primitive; angular dims get a reference arc between the reference lines with the label on the arc; labels offset off the grips (line midpoint, rect edge-mid grips). Live recompute during grip/manipulator drag; formatted through `ScaleManager` (units + precision, like `DimensionEdit`). Visible only while the item is selected; HALO sees the label only then; hover+click opens a typed-value editor (Dynamic Input HUD) that drives the geometry. NOT dimensional constraints (those come later via `parametric-constraint-system.md`). Per primitive: line = length; rect = width + height; circle = diameter; arc = included angle + radius; ellipse = major + minor radii; polyline = each segment length; polygon = radius; text = none. **Mockup-gated** (served interactive HTML before build). Reuse sweep 2026-09-24: gridline spacing dims (`Model_Space._compute_gridline_spacing` / `Model_View.drawForeground` §3c / `_start_spacing_edit`) are the pattern precedent; `DynamicInputHud` + `place_dynamic_input` are the editor; `SelectionManipulator._reflow_live` is the live-drag hook; HALO drops non-`ItemIsSelectable` candidates, so the label needs an overlay-hit step ahead of HALO ranking + manipulator press routing; primitives lack typed setters (line length, rect W/H, circle radius, arc radius/span, polyline segment). Shared component must be reusable by ALIGN §8 Selection Dimensions (nodes/sprinklers). **Mockup signed off 2026-09-24** (`FPD Design/dim-readouts`): Consolas 10px, text only (no box), aligned+readable, screen-constant, every linear label offset 14px away from the shape, no dim line, prefixes (R / R1 / R2), dashed reference arc 0.19×r min 25px with arrows, `ink` text / `muted` arcs. **Grill 2026-09-24 (refined definition):** Block Editor only (no paper follow-up); reference lines included; all selected primitives ≤ `GRIP_OBJECT_LIMIT` (none above); refresh on grip/manipulator drag, panel edit, undo/redo, units/precision (immediate repaint), zoom/pan; pick order grips > visible labels > HALO geometry; single click opens latched one-field HUD (no selection change/band/move), Enter commits / Esc+click-away cancel, red border on invalid, cancels on selection change; one undo step per commit; values — line length (keep pt1), rect local W/H (keep base edge: left/bottom), circle radius on one radial (keep centre), arc span (keep centre/radius/start; 0<θ<360) + radius on start radial, ellipse R1=rx/R2=ry (no major/minor), polyline segment lengths + vertex angles ≤180° side (0<θ≤180; closed = closing segment + all vertices, wrap; move end vertex only; skip zero-length), polygon stored defining radius (vertex radial if inscribed, edge-mid radial if circumscribed); no rotation readouts; hide labels that don't fit (and unpickable), no overlap solver; hidden during placement/transform/text-edit/placement HUD; clickable with HALO off; transient (no serialization, instances, thumbnails, snap, bounding rects). Fold-ins: promote `_format_span` → `ScaleManager`; panel Length/Width/Height/Radius/Span rows → editable formatted dimension rows via the same undoable setters, ellipse rows → R1/R2 (updates `test_dynamic_input_parity.py` Span-row assertion — deliberate contract change). Tests: pure value/format/setter units; integration on real Block Editor scene with shown view + posted events; pixel-sampled render both themes; mutation-proof guards; live smoke. `geometry_2d.py`, `model_view.py`, `model_space.py`, `halo_selection.py`, `halo.py`, `selection_manipulator.py`, `dynamic_input.py`, `scale_manager.py`, `theme.py`. ref: 2d-geometry, selection-mode, align-placement §5/§8, units-and-formatting.
...
- [ ] [type:bug] Suspected: trim/break/fillet on ArcItem mix Y-down atan2 with the arc's Y-up angles [P3] [subject:CAD]
...
- [ ] [type:bug] Suspected: arc mirror keeps start_deg and negates span (wrong for any axis) [P3] [subject:CAD]
...
- [ ] [type:maint] Native crash when running the scene_tools-area tests in one `-q` process [P3] [subject:Testing]
  - Details: reported by an implementer 2026-09-24 (exit 127 around `test_scene_tools` in a large targeted batch; passes with `-v` and in the full suite, which was green 5069/2). Unverified — gather native-crash evidence first (faulthandler) per the full-suite crash playbook.
- [ ] [type:feature] Rotate transform in the modification schema (replaces the retired rotate knob) [P2] [subject:UX]
