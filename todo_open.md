...
- [ ] [type:feature] Inline double-click text edit live in the scene (model / Block-Editor) [P1] [subject:UX]
  - Details: user, 2026-09-22 (text-panel smoke). Double-click a `TextItem` on the model / Block-Editor canvas to enter edit mode and type **live inline in the scene** (not just via the panel Content box). Blocked by the known live-only caret/paint bug: while a focused `QGraphicsTextItem` blinks its cursor, Qt calls `paint` with an engine-less viewport device so the live text + caret don't render (`project_qpainter_engineless_qgraphicstextitem`; sibling model-note bug in "UI follow-ups" §L75-76). The display fix (todo #62) renders glyphs via `_glyph_outline_local` fill, but the EDIT caret + live keystroke echo still need a real root-cause (QGraphicsView viewport update mode / text-control repaint path) — its own grill→systematic-debugging on the running app. Wire `begin_edit()`/`TextEditorInteraction` to a double-click on the model surface once the caret renders. **Also required (user smoke 2026-09-22, todo #70):** while a text item is in edit mode, app keyboard shortcuts (tool letters etc.) still fire and hijack typing. Accept `QEvent.ShortcutOverride` for printable/edit keys while a `TextItem`/`QGraphicsTextItem` has focus (see memory `project_qt_shortcutoverride_delete`, and `project_paper_undo_shortcut_routing` for Ctrl+Z inside the editor vs scene undo). Double-click-into-box issues were reproduced by the user in the same smoke. `firepro3d/text_item.py` (`begin_edit`/`paint`), `firepro3d/model_view.py`, `firepro3d/model_space.py`. ref: text-annotation-system, selection-manipulator.

## UI follow-ups

> From the 2026-09-05 UI-cleanup batch.

- [ ] [type:bug] Model-space text notes show no text + no edit cursor live, `QPainter engine==0` spam [P2] [subject:UX]
  - Details: user, 2026-09-11 (surfaced during the NoteAnnotation box-native smoke) — **PRE-EXISTING, NOT the U3 migration**: reproduces identically on `main` (32 `QPainter::begin: Paint device returned engine == 0, type: 3` + `Painter not active` bursts with zero migration code loaded). Root cause traced: while a focused `NoteAnnotation` (`QGraphicsTextItem`, `TextEditorInteraction`) blinks its text cursor, Qt calls `NoteAnnotation.paint` with an **engine-less device** (`painter.device().paintEngine() is None`, a `QWidget` viewport, `devType=1`; the "type: 3" in the warning is the inner cache image) via `model_view.event`→`super().event()`→[Qt]→`paint` — so the live text/cursor never render (static `scene.render`/`view.grab` to a valid device DO render the text — 192px — which is why it's a *live-only* bug). **Band-aid rejected 2026-09-11:** guarding `paint` with `if dev.paintEngine() is None: return` silenced the `engine==0` spam in a repro BUT calling `paintEngine()` on a `QWidget` triggers Qt's own `QWidget::paintEngine: Should no longer be called` spam (worse) and did not restore live text — reverted. Needs a real root-cause (why Qt renders the focused text item outside a valid paintEvent — QGraphicsView viewport update mode / text-control cursor repaint path), its own grill→systematic-debugging. Live-only; needs the running app + a live readout at the paint device. `firepro3d/annotations.py` (`NoteAnnotation.paint`), `firepro3d/model_view.py`, `firepro3d/model_space.py` (`_press_add_text` sets `TextEditorInteraction`).
...
- [ ] [type:design] Redefine reference/annotation graphics (gridlines, elevation markers, section cuts, detail markers, …) as block-backed composites [P3] [subject:Architecture]
  - Details: user, 2026-09-21 (HALO trace-polish smoke). Insight: a `BlockInstance` has no child items and its `shape()` = union of every render-op path, so it "falls under the rules" for free — the HALO composite trace, snap, display-manager, etc. all compose over its primitives automatically. These reference/annotation graphics are hand-rolled composite items each re-implementing parts of that (own paint, own shape, own selection chrome), so each needs bespoke handling (e.g. the `halo_trace_path` hook added for gridlines this session). Goal: back them with block definitions so they inherit composite behaviour. **Blocker to resolve first:** blocks are static pose-only (position+rotation), but these are *parametric* (gridline: origin/length/angle, auto-labels, per-end bubble visibility, lock, grips, dimension/snap participation) and have *screen-fixed* sub-parts (`GridBubble` = `ItemIgnoresTransformations`, constant 14px) that a scene-unit render-op can't express — so this needs a "parametric/screen-fixed block" model, not a literal conversion. Until then, the `halo_trace_path` hook (`halo.py:halo_scene_path`) is the general composite-trace escape hatch each such item implements. `gridline.py`, `view_marker.py`, `block_instance.py`, `block_definition.py`, `halo.py`. ref: `block-system.md`, `selection-mode.md §4.2`.
...
- [ ] [type:feature] Block-editor import: preserve partial ellipses (elliptical arcs) as a curve primitive [P3] [subject:CAD]
  - Details: deferred from the 2026-09-15 curve-fidelity task (arc/full-ellipse/spline shipped). DXF partial ELLIPSE (start_param≠end_param) still tessellates to `path_points` because no elliptical-arc primitive exists (`EllipseItem` is always-closed, §3.5.1). Needs a new parametric elliptical-arc `Geometry2DMixin` item first (centre/rx/ry/rotation/start_param/end_param), then a worker `ellipse_arc` native emission gated on `preserve_curves` + a `geom_dicts_to_primitives` branch. `geometry_2d.py`, `dxf_import_worker.py`, `geometry_import.py`. ref: 2d-geometry §3.5.3, block-system.
- [ ] [type:feature] Block-editor import: preserve PDF Bézier curves instead of flattening [P3] [subject:CAD]
  - Details: deferred from the 2026-09-15 curve-fidelity task. `pdf_import_worker` flattens all cubic Béziers to `path_points` (via `_flatten_bezier`); reconstructing editable `SplineItem`s means curve-fitting discrete Béziers into a NURBS (PDF carries no entity/layer semantics). Disproportionate vs DXF (the real curve source) — do only if PDF-authored block content proves common. `pdf_import_worker.py`, `geometry_import.py`. ref: 2d-geometry §3.5.3.
...
- [ ] [type:feature] Per-item hatch scale/density knob for 2D fills [P3] [subject:CAD]
  - Details: legacy manual angle/spacing was dropped for named patterns; a per-item hatch scale knob (like `draw_section_hatch`'s `hatch_scale`) if patterns read too dense/sparse at architectural scale. `geometry_2d.py`.
...
- [ ] [type:feature] Ctrl/from-centre + Shift-constrained resize for ROTATED rectangles [P3] [subject:CAD]
...
- [ ] [type:bug] Stale NoteAnnotation manipulator tests still expect the `scale` capability [P2] [subject:Testing]
  - Details: found 2026-09-22 in the todo #70 full suite; A/B-proven PRE-EXISTING on `main` (1c7c96a). `tests/test_manip_badge_annotations.py::test_note_annotation_moves_via_manipulator` (`assert item_capabilities(note) == {"translate","scale","rotate"}` → got no `scale`) and `tests/test_manip_griphandle_note_parity.py::test_caps_are_box_native_when_unrotated`. Likely #62 (model text dropped `scale` for font-constant grip resize). Decide: update the tests to the new capability contract, or restore `scale` for NoteAnnotation if the drop over-reached. ref: text-annotation-system, selection-manipulator.
