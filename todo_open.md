...
- [ ] [type:design] Ctrl-constrained polygon placement: close-near-first on the constrained tip or the raw cursor? [P3] [subject:UX]
  - Details: found 2026-09-24 (snap-polish G3 review S2-2). Polyline, floor and roof placement test close-near-first (and roof vertex-pop) against the RAW cursor while the committed vertex is the Ctrl-constrained tip. Probe: constrained tip 13 mm from vertex 0 (inside close tol) with raw 212 mm away → adds a near-duplicate vertex instead of closing. Decide one rule for all three (likely: test the point that would be committed). `geometry_drawing_controller.py`, `model_space.py`.
...
- [ ] [type:maint] `BlockDefinition.origin` as a cache-invalidating property [P3] [subject:Architecture]
  - Details: found 2026-09-24 (snap-polish G2 review S1). Assigning `origin` directly leaves both `_render_ops` and `_text_snap_pts` stale; today the only runtime origin change (`model_space.py` block origin-pick) calls `set_primitives` next line, so it's latent. Make `origin` a property that clears both caches. `block_definition.py`.
