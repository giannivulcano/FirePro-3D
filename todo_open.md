...
- [ ] [type:bug] Block Editor text is never compiled into saved blocks — `gather_primitives` omits `_texts` [P2] [subject:CAD]
  - Details: seam review 2026-09-23, PROVEN pre-existing on main (`git show main:firepro3d/block_editor.py` identical). `BlockEditorWidget.gather_primitives()` collects `_draw_*`/`_polylines`/`_draw_polygons`/printed reference lines but NOT `_texts`, although seeding maps TextItem → `_texts` and block definitions support "text" (C5.4 `render_outline_path`). A Block Editor with only text hits save()'s "no geometry" early return. `firepro3d/block_editor.py`.
...
- [ ] [type:bug] Test-suite access violation: GC collects a leftover MainWindow whose `main.py` lambda slot fires [P2] [subject:Testing]
  - Details: 2026-09-23, reproduces on main (A/B). Cross-module runs (e.g. `pytest tests -k "halo or undo"`) segfault with the faulthandler frame `Garbage-collecting → main.py <lambda>` — during a test's GC (main) or the `shown_model_view` teardown's `gc.collect()` (branch). A MainWindow left alive by an earlier module (module-scoped singletons) is collected later and one of its lambda-connected slots runs on a half-destroyed object. Chunked separate-process runs stay green (full-suite crash playbook). Fix direction: deterministic MainWindow teardown (disconnect lambda slots / `sip.delete` / processEvents + gc in the singleton fixtures). `tests/conftest.py`, MainWindow singleton fixtures, `main.py` lambda connects.
