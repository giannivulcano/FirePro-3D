"""Posted-event vertex-grip drag of a real RoofItem == legacy apply_grip
...
def _make_roof():
    return RoofItem([QPointF(0, 0), QPointF(100, 0), QPointF(100, 100)])


def test_roof_manip_handles_shape():
    rf = _make_roof()
    hs = rf.manip_handles()
    ...
    # house rule: vertex grips render round (disc); roof has only vertices
    assert all(h.circular is True for h in hs)
    # each rides its grip point
    for i, h in enumerate(hs):
        assert h.scene_position(None) == rf.grip_points()[i]
...
def test_posted_drag_moves_vertex(qapp):
    ...
    qapp.processEvents()
    moved = rf.grip_points()[1]
    assert abs(moved.x() - 140) < 1e-6
    assert abs(moved.y() - 20) < 1e-6
    # the other vertices are untouched
    assert rf.grip_points()[0] == QPointF(0, 0)
    assert rf.grip_points()[2] == QPointF(100, 100)
