...
def _prepped_dialog(n_geoms=1) -> UnderlayImportDialog:
    ...
    dlg._layers = ["0"]
    # Zero the rotation before building the preview (defensive: fresh dialogs
    # start at 0° now that rotation is no longer persisted to QSettings, but a
    # rotated preview group would move geometry out from under the
    # scene-coordinate snap queries below).
