...
# ---------------------------------------------------------------------------
# Test 3: rect grip resize posted == slot (BR corner grip 4, RM edge grip 3)
# ---------------------------------------------------------------------------
# Migrated (arc-rect-grip-polish Task 9): a RectangleItem no longer exposes the
# rigid RESIZE handles — it resizes through its own local-frame RectGripHandles
# at every angle. Same intent/numbers: BOTTOM_RIGHT → grip 4, RIGHT → grip 3.

@pytest.mark.parametrize("index", [4, 3])
def test_resize_posted_matches_slot(qapp, scene_and_view, index):
    """A rect grip resize driven by POSTED QMouseEvents must produce
    byte-identical serialisation to the same resize driven at slot level
    (_begin_handle/_update/_finish).
    ...
    The posted press must land on the grip's _HandleItem so _begin_handle fires
    — not fall through to selection and clear the item.  Evidence: the posted
    result differs from before-gesture AND equals the slot result.
    """
    scene, view = scene_and_view
    from firepro3d.geometry_2d import RectangleItem
    from firepro3d.manip_handle import RectGripHandle

    def _grip(manip, item):
        return next(h for h in manip._active_handles()
                    if isinstance(h, RectGripHandle) and h.item is item
                    and h.index == index)
    ...
    start = QPointF(rect_a.grip_points()[index])
    ...
    manip._begin_handle(_grip(manip, rect_a), start, start)
    ...
    initial = json.dumps({"type": "draw_rectangle",
                          ...
                          "color": rect_a.pen().color().name(),
                          "lineweight": rect_a.pen().widthF()},
                         sort_keys=True)
    assert ser_a != initial, "slot-level grip resize left geometry unchanged"
    ...
    # POST: press on the grip → _HandleItem.mousePressEvent → _begin_handle
    _post_mouse(view, QEvent.Type.MouseButtonPress, rect_b.grip_points()[index])
    assert rect_b.isSelected(), "posted press deselected the item before gesture"
    assert manip._mode == "grip", (
        f"posted press did not start a grip gesture (mode={manip._mode!r}); "
        "the grip was not hit")
