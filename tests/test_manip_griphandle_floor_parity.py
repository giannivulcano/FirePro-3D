"""Posted-event vertex-grip drag of a real FloorSlab == legacy apply_grip
outcome; one undo per gesture; Esc restores; snap parity (grid fallback).

U3 migration of FloorSlab onto manip_handles(). Mirrors the PolylineItem parity
file (test_manip_griphandle_polyline_parity.py); floor boundary vertices have no
special drag semantics (no Ctrl-constrain, no move-centre grip, no sibling
propagation) — polygons are excluded from the legacy Ctrl-constrain block."""
...
def _make_floor():
    return FloorSlab([QPointF(0, 0), QPointF(100, 0), QPointF(100, 100)])


def test_floor_manip_handles_shape():
    fl = _make_floor()
    hs = fl.manip_handles()
    assert len(hs) == 3                               # one per boundary vertex
    ...
    # house rule: vertex grips render round (disc); floor has only vertices
    assert all(h.circular is True for h in hs)
    # each rides its grip point
    for i, h in enumerate(hs):
        assert h.scene_position(None) == fl.grip_points()[i]
...
def test_posted_drag_moves_vertex(qapp):
    ...
    qapp.processEvents()
    moved = fl.grip_points()[1]
    assert abs(moved.x() - 140) < 1e-6
    assert abs(moved.y() - 20) < 1e-6
    # the other vertices are untouched
    assert fl.grip_points()[0] == QPointF(0, 0)
    assert fl.grip_points()[2] == QPointF(100, 100)
