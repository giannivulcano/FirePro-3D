...
def test_grips_are_manipulator_owned(qapp):
    # U3: the legacy _PullTabGrip children are gone; grips are provided as
    # GripHandles via manip_handles() and rendered by the manipulator with the
    # house selection style (grip styling is no longer gridline-owned).
    from PyQt6.QtCore import QPointF
    from firepro3d.gridline import GridlineItem
    gl = GridlineItem(QPointF(0, 0), QPointF(0, 1000), label="A")
    assert not hasattr(gl, "_grip1")
    assert len(gl.manip_handles()) == 4
